...
Player._getClearpipeDirection = getDirection
...
local function turnJunction(pipe, i, p)
    ...
    if key == 0 then
        ...
        for _,bgo in ipairs(BGOGetIntersecting(pipe.x, pipe.y, pipe.x + pipe.width, pipe.y + pipe.height)) do
    ...
    if #inPipe > 0 then
        for i, p in ipairs(Player.get()) do
            if inPipe[i] == 2 then
                ...
                for _,bgo in ipairs(BGOGetIntersecting(p.x, p.y, p.x + p.width, p.y + p.height)) do
