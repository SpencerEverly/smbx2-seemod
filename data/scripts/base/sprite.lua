...
local function getCurrentFrame(f)
    if f == nil then
        return 1,1
    end
...
local function spriteDraw(s, args)
    ...
    if s.texture ~= nil then
        ...
        local fx,fy = getCurrentFrame(args.frame)
        local frameSpeed = args.frameSpeed or 1
    ...
    if args.scene ~= nil then
        drawtbl.sceneCoords = args.scene
    elseif args.sceneCoords ~= nil then
        drawtbl.sceneCoords = args.sceneCoords
    end
    drawtbl.texture = s.texture
    drawtbl.primitive = s.__basis.primitive
    drawtbl.color = drawtbl.color or args.color
    drawtbl.priority = drawtbl.priority or args.priority
