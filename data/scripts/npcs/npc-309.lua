...
function montyMoles.onTickNPC(v)
	...
	elseif data.state == ST_CHASE then
		if not (v:mem(0x12C, FIELD_WORD) > 0 or v:mem(0x132, FIELD_WORD) > 0 or v:mem(0x134, FIELD_WORD) > 0) then --chase
			chasePlayers(v)
			v.speedX = math.clamp(v.speedX + 0.075 * data.direction, -4*NPC.config[npcID].speed, 4*NPC.config[npcID].speed)
			data.timer = data.timer - 1
			if data.timer <= 0 and v.collidesBlockBottom then
				v.speedY = RNG.random(-2, -4)
