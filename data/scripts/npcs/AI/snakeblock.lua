...
local BGOGetIntersecting

local newBGOSystem
pcall(function() newBGOSystem = require("see/newBGOSystem") end)

if newBGOSystem == nil then
    BGOGetIntersecting = BGO.getIntersecting
else
    BGOGetIntersecting = newBGOSystem.getIntersecting
end
...
local function doSpeedStuff(self)
	self.speedY = self.data._basegame.vector[2] * NPC.config[self.id].basespeed * NPC.config[self.id].speed
	if self.speedY > 8 then
		self.y = self.y + self.speedY - 8
	end
	if self.speedY == 0 or NPC.config[self.id].altdiagonalmovement then
		self.speedX = 0
		self.x = self.x + self.data._basegame.vector[1] * NPC.config[self.id].basespeed * NPC.config[self.id].speed
	else
		self.speedX = self.data._basegame.vector[1] * NPC.config[self.id].basespeed * NPC.config[self.id].speed
	end
end
...
function snakeBlock.onTick()
	if needsChecking then
		for _,p in ipairs(Player.get()) do
			for _,v in ipairs(Block.getIntersecting(p.x,p.y+p.height,p.x+p.width,p.y+p.height+1)) do
...
function snakeBlock.onTickNPC(npc)
	...
	if not data.active then
		...
		delta.x = delta.x + math.abs(data.vector[1]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
		delta.y = delta.y + math.abs(data.vector[2]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
		...
		if delta.x >= data.width or delta.y >= data.height then
			...
			if sect.isLevelWarp then
				if point.x < sect.boundary.left then
					...
					npc.x = point.x
					delta.x = math.abs(data.vector[1]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
			...
			for _,bgo in ipairs(BGOGetIntersecting(npc.x,npc.y,npc.x+data.width,npc.y+data.height)) do
				pnt.x = bgo.x + 16
				pnt.y = bgo.y + 16
			...
			for _,id in ipairs(cids) do
				if not hasRedirected then
					...
					if redirector.VECTORS[id] ~= vector.zero2 then
						...
						npc.y = point.y
						delta.x = math.abs(data.vector[1]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
						delta.y = math.abs(data.vector[2]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
					end
					hasRedirected = true
				elseif hasRedirected then
					...
					newNPC.data._basegame.delta.x = math.abs(data.vector[1]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
					newNPC.data._basegame.delta.y = math.abs(data.vector[2]) * NPC.config[npc.id].basespeed * NPC.config[npc.id].speed
					if redirector.VECTORS[id].x == -origVector[1] and redirector.VECTORS[id].y == -origVector[2] then
						if npc.direction == BHV_EAT then
			...
			if npc.direction == BHV_EAT then
				local iBlockRefs = {}
				for k,v in ipairs(Block.getIntersecting(point.x+2,point.y+2,point.x+npc.width-2,point.y+npc.height-2)) do
