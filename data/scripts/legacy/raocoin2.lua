...
function raocoin.registerCurrency(npcID, showUI, UIx, UIy, imgPath, ignoreHud)
	...
	UIx = UIx or 0;
	UIy = UIy or 0;
	imgPath = imgPath or getSMBXPath().."\\scripts\\Legacy\\raocoin\\dragoncoin.png";
...
raocoin.IMG_TOKEN_T = Graphics.loadImage(getSMBXPath().."\\scripts\\Legacy\\raocoin\\token_t.png");
raocoin.IMG_TOKEN_P = Graphics.loadImage(getSMBXPath().."\\scripts\\Legacy\\raocoin\\token_p.png");
raocoin.IMG_ARROW = Graphics.loadImage(getSMBXPath().."\\scripts\\Legacy\\raocoin\\arrow.png");
	...
	for k,v in pairs(tokens) do
		if(not k:isBought()) then
			...
			if(v.npc == nil and v.npcID ~= nil and v.enableTimer <= 0) then
				n = spawnNPC(v.npcID,v.x,v.y-70-64,v.section);
			...
			if(v.enableTimer <= 0) then
				...
				if(colliders.collide(player,v.hitbox)) then
					...
					if(tryBuy) then
						...
						sourceY = v.y;
						if(k:canAfford()) then
							Audio.playSFX(getSMBXPath().."\\scripts\\Legacy\\raocoin\\buy.ogg");
						...
						if(v.npc ~= nil) then
							...
							Audio.playSFX(getSMBXPath().."\\scripts\\Legacy\\raocoin\\fail.ogg");
						end
						buyAction = k;
					end
				end
...
function raocoin.drawhud()
		...
		for k,v in pairs(currencies) do
			if(v.visible and (v.ignoreHud or (not isOverworld and Graphics.isHudActivated()))) then
