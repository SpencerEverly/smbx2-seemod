...
__disablePauseMenu - Set this as true whenever to disable the pause menu. !!!Unfortunately, this is the only way with eventToken to disable the default pause menu--eventToken.cancelled won't only just work with this. You'll need to both do eventToken AND __disablePauseMenu to disable it completely!!!
Example:

__disablePauseMenu = true
local onPause(eventToken)
...
Misc.setWindowIcon(Graphics.loadImageResolved("string/to/file.extension")) - Loads an image as an icon in the episode folder and replaces it with the window icon with Graphics.loadImageResolved. Graphics.loadImage can also be optionally used.
Misc.saveDataManage(slot, newslot..optional, destination..optional, action, booltocountext, booloverwrite..optional) - A stripped down version of os.remove/os.rename that manages save files.
...
Misc.openComputerFile(extension, localtoassign) - Opens up the Windows Explorer open file dialog to open a file. This only accepts certain files (Graphics, music, lua files), which you can specify (Which is also limited).
...
Misc.loadScreenFinished() - Checks to see if the loadscreen is finished loading everything. Useful for fade outs before starting the level.
